...
with st.beta_expander('Application Information'):
    st.markdown('''This is a tweet reply sentiment extractor, and it will assist in retrieving
            ...
            Examples:
                ...
                🚀 -> rocket
            ''')
current_model = joblib.load('model_clf.pkl')
...
# PREDICTION
    ...
    input_df['Text'] = input_df.Text.apply(text_cleaner.emoji_to_text)
...
# DATA WINDOW
    ...
    with col1:
        ...
        if len(error_number) > 0:
            ...
            sentiment = []#w ill be used to partially fit corrections to model
            for i in error_number:
                if input_df['Sentiment'].iloc[i] == 'Positive':
                    # Flips sentiment (positive to negative):
                    input_df['Sentiment'].iloc[i] = 'Negative'
                    # Append corrections to be used for partial-fit:
                    text.append(input_df['Text'].iloc[i])
                    sentiment.append(input_df['Sentiment'].iloc[i])
                    placeholder.write(input_df)
                else:
                    # Flips sentiment (negative to positive):
                    input_df['Sentiment'].iloc[i] = 'Positive'
                    # Append corrections to be used for partial-fit:
                    text.append(input_df['Text'].iloc[i])
                    sentiment.append(input_df['Sentiment'].iloc[i])
                    placeholder.write(input_df)
            ...
            joblib.dump(current_model, 'model_clf.pkl') # updating pickled model
    ...
    input_df['Clean'] = input_df.Text.apply(text_cleaner.clean)
